...
import { ARTICLES_INDEX, CATEGORY_STYLES } from "@/data/writing&insight";
...
const CATEGORIES = [
  ...
  { label: "Preventive Health Innovation" },
  { label: "AI & Regulated Data" },
];
...
export default function WritingPage() {
  ...
  const filtered = useMemo(
    () => activeCategory === "All" ? ARTICLES_INDEX : ARTICLES_INDEX.filter((a) => a.category === activeCategory),
    [activeCategory]
  );
    ...
    <main className="relative w-full overflow-x-hidden text-sm text-zinc-200">
      ...
      <section className="bg-black">
        <div className="mx-auto max-w-6xl px-4 pt-24 pb-16 sm:px-6 md:px-10 lg:px-0 text-center">
          <h1 className="text-5xl font-bold text-white md:text-6xl lg:text-7xl">Writing &amp; Insights</h1>
        ...
        <div className="mx-auto max-w-6xl px-4 py-4 sm:px-6 md:px-10 lg:px-0">
          ...
          >
            {CATEGORIES.map(({ label }) => {
              const count = label === "All" ? ARTICLES_INDEX.length : ARTICLES_INDEX.filter((a) => a.category === label).length;
              return (
