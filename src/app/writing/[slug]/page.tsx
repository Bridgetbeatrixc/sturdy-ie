...
import { ARTICLES_DETAIL, CATEGORY_STYLES } from "@/data/writing&insight";

export function generateStaticParams() {
  return Object.keys(ARTICLES_DETAIL).map((slug) => ({ slug }));
}
...
export default async function ArticleDetailPage({
  ...
  const article = ARTICLES_DETAIL[slug];
  if (!article) notFound();
  ...
  return (
    <main className="relative w-full overflow-x-hidden bg-black text-zinc-200">
      ...
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-0">
        {/* Back */}
        <div className="py-8">
          <Link href="/writing" className="inline-flex items-center gap-2 text-xs text-zinc-500 hover:text-white transition-colors">
